...
source("./R_functions/gExplode.r")
source("./R_functions/gHoles.r")
...
proj4string(mapa_app_explode)

gI <- gIntersects(mapa_app_explode, micro, byid = TRUE )
...
```{r}
proj4string(mapa_APP)
proj4string(mapa_CAR)

st_crs(mapa_APP)
proj_crs<-CRS(SRS_string = "+init=epsg:31997")


mapa_APP <- spTransform(mapa_APP, crs("+init=epsg:31997")) 
mapa_CAR <- spTransform(mapa_CAR, crs("+init=epsg:31997"))



mapa_APP<-gBuffer(mapa_APP, byid = TRUE, width = 0)
mapa_app_explode<-gExplode(mapa_APP)
mapa_app_holes<-gHoles(mapa_app_explode,)
...
#x11()
plot(mapa_APP)
plot(mapa_APP[5,], border= "red", col="red", add=TRUE)
axis(1)
```
