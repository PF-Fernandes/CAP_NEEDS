...
#### Acessando a tabela de atributos:
```{r}
View(CMA_car@data)
```
...
```{r}
str(CMA_car@data$NUM_MODULO)
```

#### Quantidade de imoveis de pequenas propriedades:
```{r}
sum(peq_prop)
(232/257)*100 # Porcentagem
```

#### Quantidade de imoveis de médias propriedades:
```{r}
sum(med_prop&med_prop1)
(16/257)*100 # Porcentagem
```

#### Quantidade de imoveis de grandes propriedades:
```{r}
sum(gra_prop)
(9/257)*100 # Porcentagem
```
...
```{r}
ha_peq <- CMA_car[CMA_car@data$NUM_AREA <= 88.000000,]
sum(ha_peq@data$NUM_AREA)

(3634.191/15075.57)*100 # Porcentagem
```

#### Quantidade total de área/hectares de médias propriedades:
```{r}
ha_med <- CMA_car@data$NUM_AREA > 88.000000
ha_med1 <- CMA_car@data$NUM_AREA <= 330.000000
ha_med2 <- CMA_car[ha_med&ha_med1,]
sum(ha_med2@data$NUM_AREA)

(2890.142/15075.57)*100 # Porcentagem
```

#### Quantidade total de área/hectares de grandes propriedades:
```{r}
ha_gra <- CMA_car[CMA_car@data$NUM_AREA > 330.000000,]
sum(ha_gra@data$NUM_AREA)

(8551.24/15075.57)*100 # Porcentagem
```
